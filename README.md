...
│   ├── retrieval/                decontextualizer.py, reranker.py (sigmoid-calibrated)
...
│   ├── metrics.py                Post-output grounding, IR (Recall@K/Precision@K/MRR), Agent trajectory & TNR/FPR
│   ├── llm_judge.py              Decomposed 5-criterion rubric-anchored LLM-as-a-judge (+ variance & SME flags)
...
- **Statutes:** if `data/raw/in_central_legislation.parquet` is present (downloaded from the [open-india-law](https://huggingface.co/datasets/vaquill/open-india-law) mirror), sections from the target Acts are extracted with DuckDB. In addition, a curated set of **40 statutory provisions** (`MV Act, BNS, BNSS, IPC, CrPC, TPA, SRA, CPC, CPA 2019`) with verified `https://indiacode.gov.in/handle/123456789/...` links is always merged in so 100% of benchmark-referenced sections are present.
- **Precedents:** a curated set of **6 landmark Supreme Court judgments** with verified Indian Kanoon (`/doc/<id>/`) links ([`data/ingestion/normalizer.py`](data/ingestion/normalizer.py)).
...
## Evaluation Benchmark (52 Scenarios — Full OpenAI Results)

- **Dataset:** [`evaluation/benchmark_dataset.json`](evaluation/benchmark_dataset.json) has **52 annotated scenarios**: 15 motor vehicle, 15 property, 15 consumer, and 7 out-of-scope/adversarial guardrail probes. Each has gold-standard domain, forum, `expected_sections`, `critical_steps`, and `expects_criminal_route` metadata.
- **Multi-Layer Evaluation:**
  - **Post-Output Citation Grounding & Pre-Verification Strip Rate:** Symmetric grounding verification across Approach A and Approach B against retrieved chunks + exact section lookup in Qdrant.
  - **Retrieval IR Metrics:** Section Recall@K, Precision@K, Mean Reciprocal Rank (MRR), and Context Relevance.
  - **Agentic Control-Flow & Guardrail Metrics:** Node Trajectory Validity, Criminal Routing Accuracy, Guardrail Out-of-Scope TNR / In-Scope FPR, and End-to-End Task Success Rate.
  - **Decomposed 5-Criterion LLM-as-a-Judge + SME Calibration:** Parallel rubric-anchored evaluation (`statutory_accuracy`, `procedural_actionability`, `forum_appropriateness`, `hallucination_freedom`, `coherence_and_specificity`) with SME calibration (`evaluation/sme_review.py` & `evaluation/sme_annotations.json`).

```bash
python -m solvemycase.evaluation.comparative_runner                       # all 52 scenarios
...
| Evaluation Metric (52 Scenarios) | Approach A (Baseline RAG) | Approach B (Proposed LangGraph) |
|---|---:|---:|
| **Post-Output Hallucination Rate** | 3.6% | **0.0%** |
| **Citation Grounding Accuracy** | 83.0% | **100.0%** |
| **Pre-Verification Strip Rate** | 0.0% | **1.3%** |
| **Expected Section Recall (Final)** | 44.4% | **51.7%** |
| **Retrieval Mean Reciprocal Rank (MRR)** | 0.96 | **0.90** |
| **Procedural Phase Completeness (0–1)** | 0.68 | **0.90** |
| **Designated Forum Accuracy** | 59.6% | **88.5%** |
| **Agent Trajectory Validity** | 86.5% | **100.0%** |
| **Guardrail Out-of-Scope TNR** | 0.0% | **100.0%** |
| **End-to-End Task Success Rate** | 59.6% | **88.5%** |
| **LLM Judge Overall Rating (0–5)** | 3.63 | **4.53** |
| **Mean End-to-End Latency** | 5.11s | 10.30s |
...
The suite (**53 tests**) covers config, normalizer (100% benchmark statutory coverage), vector store, both pipelines (including `run_with_trace` and streaming), symmetric grounding/IR/agent/noise-sensitivity/SME evaluation metrics, decomposed LLM judge, the FastAPI service, and every Streamlit UI page (offline `AppTest`).
...
| Phase | Scope | Status |
|---|---|---|
| 1–4 | Scaffold, config, 46-item corpus ingestion, hybrid Qdrant store, guardrail, decontextualizer, sigmoid-calibrated reranker, `RetrievalQualityGate`, planner, conjunctive verification, LangGraph | ✅ Done |
| — | BNS/BNSS/IPC criminal-code RAG sub-layer + citation re-grounding + runtime JSONL telemetry | ✅ Done |
| 5 | 52-scenario OpenAI benchmark, RAG/Agent/Guardrail metrics, decomposed 5-dimension LLM judge, SME calibration & noise-sensitivity harness | ✅ Done |
| 6 | FastAPI service + Streamlit side-by-side UI (with 3-tab Benchmark & SME calibration dashboard) | ✅ Done |
